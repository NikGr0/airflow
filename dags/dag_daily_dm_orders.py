...
dag = DAG('dag_daily_dm_orders', default_args=default_args, schedule_interval='0 * * * *', catchup=True,
          max_active_tasks=3, max_active_runs=1, tags=["data marts", "dm_orders"])

clear_day = PostgresOperator(
    task_id='clear_day',
    postgres_conn_id='main_postgresql_connection_test3',
...
calc_day = PostgresOperator(
    task_id='calc_day',
    postgres_conn_id='main_postgresql_connection_test3',
    sql="""insert into public.dm_orders(buy_date, product_name, order_city_name, customer_name, account_number, customer_city_name, total_value, total_costs)
